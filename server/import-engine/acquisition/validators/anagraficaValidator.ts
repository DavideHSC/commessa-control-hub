...
export function validateAnagrafica(rawData: any): ValidatedAnagrafica {
  ...
  const mapped = {
    // Dati azienda  
    codiceFiscaleAzienda: rawData.CODICE_FISCALE_AZIENDA || rawData.codiceFiscaleAzienda || '',
    subcodeAzienda: rawData.SUBCODICE_AZIENDA || rawData.subcodeAzienda || '',
    
    // Dati generali
    codiceUnivoco: rawData.CODICE_UNIVOCO || rawData.codiceUnivoco || '',
    codiceFiscaleClifor: rawData.CODICE_FISCALE_CLIFOR || rawData.codiceFiscaleClifor || '',
    subcodeClifor: rawData.SUBCODICE_CLIFOR || rawData.subcodeClifor || '',
    tipoConto: rawData.TIPO_CONTO || rawData.tipoConto || '',
    sottocontoCliente: rawData.SOTTOCONTO_CLIENTE || rawData.sottocontoCliente || '',
    sottocontoFornitore: rawData.SOTTOCONTO_FORNITORE || rawData.sottocontoFornitore || '',
    codiceAnagrafica: rawData.CODICE_ANAGRAFICA || rawData.codiceAnagrafica || '',
    partitaIva: rawData.PARTITA_IVA || rawData.partitaIva || '',
    tipoSoggetto: rawData.TIPO_SOGGETTO || rawData.tipoSoggetto || '',
    denominazione: rawData.DENOMINAZIONE || rawData.denominazione || '',
    
    // Dati persona fisica
    cognome: rawData.COGNOME || rawData.cognome || '',
    nome: rawData.NOME || rawData.nome || '',
    sesso: rawData.SESSO || rawData.sesso || '',
    dataNascita: rawData.DATA_NASCITA || rawData.dataNascita || '',
    comuneNascita: rawData.COMUNE_NASCITA || rawData.comuneNascita || '',
    comuneResidenza: rawData.COMUNE_RESIDENZA || rawData.comuneResidenza || '',
    cap: rawData.CAP || rawData.cap || '',
    indirizzo: rawData.INDIRIZZO || rawData.indirizzo || '',
    prefissoTelefono: rawData.PREFISSO_TELEFONO || rawData.prefissoTelefono || '',
    numeroTelefono: rawData.NUMERO_TELEFONO || rawData.numeroTelefono || '',
    idFiscaleEstero: rawData.ID_FISCALE_ESTERO || rawData.idFiscaleEstero || '',
    codiceIso: rawData.CODICE_ISO || rawData.codiceIso || '',
    
    // Dati pagamenti
    codiceIncassoPagamento: rawData.CODICE_INCASSO_PAGAMENTO || rawData.codiceIncassoPagamento || '',
    codiceIncassoCliente: rawData.CODICE_INCASSO_CLIENTE || rawData.codiceIncassoCliente || '',
    codicePagamentoFornitore: rawData.CODICE_PAGAMENTO_FORNITORE || rawData.codicePagamentoFornitore || '',
    codiceValuta: rawData.CODICE_VALUTA || rawData.codiceValuta || '',
    
    // Dati fiscali fornitore
    gestioneDati770: rawData.GESTIONE_DATI_770 || rawData.gestioneDati770 || '',
    soggettoARitenuta: rawData.SOGGETTO_A_RITENUTA || rawData.soggettoARitenuta || '',
    quadro770: rawData.QUADRO_770 || rawData.quadro770 || '',
    contributoPrevidenziale: rawData.CONTRIBUTO_PREVIDENZIALE || rawData.contributoPrevidenziale || '',
    codiceRitenuta: rawData.CODICE_RITENUTA || rawData.codiceRitenuta || '',
    enasarco: rawData.ENASARCO || rawData.enasarco || '',
    tipoRitenuta: rawData.TIPO_RITENUTA || rawData.tipoRitenuta || '',
    soggettoInail: rawData.SOGGETTO_INAIL || rawData.soggettoInail || '',
    contributoPrevid335: rawData.CONTRIBUTO_PREVID_335 || rawData.contributoPrevid335 || '',
    aliquota: rawData.ALIQUOTA || rawData.aliquota || '',
    percContributoCassa: rawData.PERC_CONTRIBUTO_CASSA || rawData.percContributoCassa || '',
    attivitaMensilizzazione: rawData.ATTIVITA_MENSILIZZAZIONE || rawData.attivitaMensilizzazione || ''
  };
